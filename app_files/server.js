...
try {
	...
	if(reqData.type == 'create_room') {
		...
		console.log(sessions[Id].playerNames)
...
app.get('/fetchsolo', function (req, res) {
	try {
		res.setHeader("Access-Control-Allow-Origin", "*");
		res.setHeader("Content-Type", "application/json");
		let toSend = JSON.parse(JSON.stringify(pack));
		toSend.result = result;
		res.end(JSON.stringify(toSend, null)); //отправляем пакет
...
app.get('/gameResults', function (req, res) {
	...
	if(reqData.results)
		resCode = 200
	teams = JSON.parse(reqData.results)

	console.log(teams)

	var teamsOld = fs.readFileSync('leaderboard.json')
	...
	res.end(`{"message": "STATUS OK"}`); //отправляем пакет
...
async function processFile(exelFile) {
	...
	app.listen(8000, () => {
		...
		exec('start http://localhost:8000', (err, stdout, stderr) => {
  			if (err) {
